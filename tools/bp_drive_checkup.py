...
def render(c: Checkup, capped: bool) -> None:
  ...
  # CHECK 15 (TSR SIGN READS) IS GONE, 2026-09-26. It printed "NOT ONE sign" on every drive
  # forever, which is the check-that-fires-every-drive failure this repo already records -- and
  # its stated job was the before/after for an as-built experiment that is now closed. The
  # stuck-constant-limit detector in check 9 STAYS: that one is on the RESOLVER side and is what
  # catches the 000003b6 failure if SpeedLimitPolicy ever moves off map-data-only.
