...
class _Icbm:
  def __init__(self, send=0, override=1, baseline=72.0, suppressed=False, source=0):
...
def test_the_badge_appears_once_a_real_limit_exists():
  assert read_icbm_hud_state(_SM2(_Icbm(baseline=72.0), _SL(True, 24.6))).has_hold
...
def test_a_valid_flag_with_no_limit_still_draws_the_hold():
  ...
  """
  assert read_icbm_hud_state(_SM2(_Icbm(baseline=72.0), _SL(True, 0.0))).has_hold


def test_no_hold_is_never_drawn_however_valid_the_limit():
  assert not read_icbm_hud_state(_SM2(_Icbm(baseline=0.0), _SL(True, 24.6))).has_hold
...
def test_missing_speed_limit_data_does_not_raise():
  ...
  assert read_icbm_hud_state(_SM2(_Icbm(baseline=72.0), None)).has_hold
